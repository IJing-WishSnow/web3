    ...
    AccessControlUpgradeable
{
    ...
    */
    ...
    function withdraw(
        ...
        for (uint256 i = popNum_; i < user_.requests.length; i++) {
            user_.requests[i - popNum_] = user_.requests[i];
        }
    ...
    //     问题在 _safeETHTransfer 函数的返回值检查逻辑有误。
    // 原因：发送 ETH 的 call 不应该返回 bool 数据，原代码中的 abi.decode(data, (bool)) 检查是错误的。
    // 如果接收方返回了非空但无法解码为 bool 的数据，会导致 revert。直接检查 success 即可。

    // 总结：
    // 1. if (data.length > 0) 确实会排除 EOA，因为 EOA 返回空数据，所以我之前说的 EOA 会 revert 是错误的
    // 2. 真正的问题： 
    // - 使用 call{value: ...}("") 发送 ETH 时，data 通常总是空的（无论接收方是 EOA 还是合约）
    // - 但如果某些特殊情况下 data.length > 0，原代码会尝试将其解码为 bool
    // - 如果 data 不是有效的 bool 编码（32字节，值为0或1），abi.decode 会 revert
    // - 即使转账已经成功（success = true），也会因为无法解码而回滚
    // 3. 根本问题：这个二次检查基于错误的假设，call 的 success 已经足够表示转账成功，检查 data 是多余且可能引发意外 revert 的
}
